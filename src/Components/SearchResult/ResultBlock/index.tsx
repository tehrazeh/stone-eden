...
const ResultBlock = () => {
  ...
  const { currentPage, elementsPerPage, infiniteScroll } = useAppSelector(
    (state) => state.pagination
  );
  const { nameFilter, sortFilter, dropdownFilters, activeFilters } =
  ...
  useEffect(() => {
    ...
    if (activeFilters.length > 0) {
      let tempItems = activeFilters.map((selectedFilter) => {
        return data.filter((card) => {
          if (card[selectedFilter as keyof Card] !== undefined) {
            return (
              card[selectedFilter as keyof Card]
                ?.toString()
                .toLowerCase()
                .split(" ")
                .join("") ===
              dropdownFilters[
                `${selectedFilter}Filter` as keyof DropdownFilters
              ]
            );
          }
        });
      ...
      const filteredData = tempItems.reduce((a, b) =>
  ...
  const cardPortion = paginateArray(
    displayedData,
    elementsPerPage,
    currentPage - 1
  );
  ...
  if (infiniteScroll) {
    // infinite scroll is on, display pile that will be updated with each scroll to footer
  ...
  if (status === Status.LOADING) {
    return (
      <div className="flex flex-wrap justify-center my-20 h-full w-full items-center">
        <FadeLoader
          color="#36d7b7"
          height={10}
          margin={2}
  ...
  return (
    <div className="w-full bg-stone-800 flex flex-wrap justify-center items-center flex-col">
      ...
      <div className="flex flex-col w-full bg-stone-800 mb-1">
        ...
        <div className="w-full flex flex-wrap justify-evenly my-1">
          {pageElements}
        </div>
        {status === Status.ERROR && (
          <div className="text-6xl pt-4 text-cyan-600">
            {status.toUpperCase()}
          </div>
        )}
        ...
        {!inView && (
          <div
            className="rounded-xl opacity-80 w-14 h-14 fixed bottom-[50px]
